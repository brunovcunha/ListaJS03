...
texto = prompt("Digite um texto que fale sobre furtas e verduras: ");

textoOriginal = texto;
palavras = textoOriginal.split(" ");
...
for (i = 0; i < texto.length; i++) {
    ...
    if (vetLegumes.includes(texto[i].toUpperCase()))
        legumes.push(texto[i]);
}

contFrutas = frutas.length;
contVerduras = legumes.length;

document.write(`<h1>Frutas contidas no texto: ${contFrutas}</h1>`);
document.write(`<p>${frutas}</p>`);

document.write(`<h1>Verduras contidas no texto: ${contVerduras}</h1>`);
document.write(`<p>${legumes}</p>`);

document.write(`<hr>`);

document.write(`<h2>Estatísticas</h2>`);
document.write(`<p>Texto original: ${textoOriginal}</p>`);
document.write(`<p>Total de palavras analisadas: ${contPalavras}</p>`);

textoOriginal = textoOriginal.replaceAll("?", ".");
textoOriginal = textoOriginal.replaceAll("!", ".");

frases = textoOriginal.split(".");

for (j = 0; j < frases.length - 1; j++) {
    document.write(`<li>${frases[j]}</li>`);
}

document.write(`<p>Total de frases analisadas: ${frases.length - 1}</p>`);
